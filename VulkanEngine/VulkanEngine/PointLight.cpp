...
namespace HN
{
	...
	PointLight::PointLight(Device& HDevice, VkRenderPass renderPass, VkDescriptorSetLayout globalSetLayout) : device{ HDevice }
	{
	...
	void PointLight::createPipeline(VkRenderPass renderPass) {
		...
		pipeline = std::make_unique<Pipeline>(device, "shader/pointlight.vert.spv", "shader/pointlight.frag.spv", pipelineConfig);
	}
}